"""Keep the duplicated Padua Aerospace record aligned with checked source data."""
...
def main() -> None:
    ...
    source = next(item for item in source_document["universities"] if item.get("id") == "unipd_aerospace")
    ...
    target = next(item for item in target_rows if item.get("id") == "it-padova-aero-msc")
    fields = [
    ...
    print("Synchronized duplicate Padua Aerospace record with official source-grounded content.")
