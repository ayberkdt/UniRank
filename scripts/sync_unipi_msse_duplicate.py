"""Keep the duplicated UniPi MSSE record source-identical across Italy datasets."""
...
def main() -> None:
    ...
    source = next(item for item in source_document["universities"] if item.get("id") == "unipi_aerospace_master")
    ...
    target = next(item for item in target_rows if item.get("id") == "it-pisa-aero-msc")
    ...
    if isinstance(target_document, dict):
        target_document["last_updated"] = CHECKED
    newline = "\r\n" if "\r\n" in original else "\n"
    TARGET_PATH.write_text(json.dumps(target_document, ensure_ascii=False, indent=2).replace("\n", newline) + newline, encoding="utf-8")
    print("Synchronized the duplicate UniPi MSSE record with the checked source-grounded record.")
