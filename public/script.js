...
// Country visuals (flag textures, accents, ISO codes) live in countryVisual.js,
// which the calendar page shares.  These are thin local names for the call
// sites below.
function applyCountryVisual(element, country) {
    if (window.uniApplyCountryVisual) window.uniApplyCountryVisual(element, country);
}

function countryFlagCode(country) {
    return window.uniCountryFlagCode ? window.uniCountryFlagCode(country) : "";
}
...
function openDrawer(data) {
    try {
        ...
        // The admission facts a reader checks first - test minimums, GRE,
        // letters, GPA floor - as tokens under the title.
        const requirementSummaryHTML = window.uniRequirementSummary ? window.uniRequirementSummary.panel(record) : '';
